...
int moving_mode = -1;  // modified in uart.cpp --> void* receive_thread(void* ptr)
...
int main()
{
    ...
    {   
		...
		else if (main_status == 11)
		{
			...
			morphologyEx(green_thres, green_thres, MORPH_OPEN, element);  //开操作
			morphologyEx(green_thres, green_thres, MORPH_CLOSE, element);  //闭操作
			vector<Point> paw_pos = get_blobs_center(green_thres, 900, 4000);
			cout << paw_pos << endl;
			if (paw_pos.size() > 0)
			{
				paw_zero_position = paw_pos[0];
				paw_zero_position.x -= 52;
				cout << paw_zero_position << endl;
				paw_outside_position = {-130-paw_zero_position.x, 0-paw_zero_position.y};
			}
			imshow("imgwrap", img_wrap_BGR);
			imshow("green_thresh", green_thres);
		}
		else if (main_status >= 20)
		{
			if (main_status == 20)
			{
				msg = "tx=" + to_string(paw_outside_position.x) +".00!\r\n";
				uart_send(fd, msg.c_str());
				usleep(10000);
				cout << msg << endl;
				msg = "ty=" + to_string(paw_outside_position.y) +".00!\r\n";
				cout << msg << endl;
				uart_send(fd, msg.c_str());
				usleep(10000);
				uart_send(fd, "id=2.00!\r\n");
				usleep(10000);
				main_status = 21;
				continue;
			}
    		...
    		subImage.setTo(0);
			black_thres(Rect(0,0,400,12)).setTo(0);
			...
			if (main_status == 21)
			{
    			...
    			subImage.setTo(0);
				black_blob_pos = get_blobs_center(black_thres, 1000, 4000);
				if (black_blob_pos.size() > 0)
				{
					if (moving_mode == 0)
					{
						//grab
						Point tempTarget = black_blob_pos[0];
						msg = "tx=" + to_string(tempTarget.x-paw_zero_position.x) +".00!\r\n";
						uart_send(fd, msg.c_str());
						usleep(10000);
						msg = "ty=" + to_string(tempTarget.y-paw_zero_position.y) +".00!\r\n";
						uart_send(fd, msg.c_str());
						usleep(10000);
						uart_send(fd, "id=1.00!\r\n");
						usleep(10000);
						moving_mode = -1;
					}
					...
					{
						//put down
						msg = "tx=" + to_string(int(50.00-paw_zero_position.x)) +".00!\r\n";
						uart_send(fd, msg.c_str());
						msg = "ty=" + to_string(int(50.00-paw_zero_position.y)) +".00!\r\n";
						uart_send(fd, msg.c_str());
						uart_send(fd, "id=2.00!\r\n");
					}
